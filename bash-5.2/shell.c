...
static void
    execute_env_file(env_file) char *env_file;
{
  char *fn;
...
/* Perhaps make this shell a `restricted' one, based on NAME.  If the
   ...
   started with -r. */
int maybe_make_restricted(name)
char *name;
{
  char *temp;
...
/* Close the current shell script input source and forget about it.  This is
   ...
   input (fd 0). */
void
    unset_bash_input(check_zero) int check_zero;
{
#if defined(BUFFERED_INPUT)
...
static void
    set_shell_name(argv0) char *argv0;
{
...
static void
    add_shopt_to_alist(opt, on_or_off) char *opt;
int on_or_off;
{
